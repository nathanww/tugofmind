...
function sendData(datavalue) {
  ...
  const url = 'http://biostream-1024.appspot.com/sendps?user=player2&data=0,'+datavalue;
      Http.open("GET",url);
      Http.send();
  ...
  client.ready.then(() => client.init()).then(() =>
    numbers(client, avgWindow, averages => {
      const output = Object.keys(averages)
        ...
        //console.log(averages['alpha'].toFixed(2));

      //console.log(output);

      const Http = new XMLHttpRequest();
      // const url = 'http://biostream-1024.appspot.com/sendps?user=player1&data=1,2,3';
      const thetaVal=averages['theta'].toString();
      const alphaVal=averages['alpha'].toString();
      const betaLVal=averages['betaL'].toString();
      const betaHVal=averages['betaH'].toString();
      const gammaVal=averages['gamma'].toString();
      const allVals=[thetaVal,alphaVal,betaLVal,betaHVal,gammaVal].join(',');
      // .concat(thetaVal).join(',').concat(alphaVal).join(',').concat(betaLVal).join(',').concat(betaHVal).join(',').concat(gammaVal);
      // const url = 'http://biostream-1024.appspot.com/sendps?user=player2&data='.concat(allVals);
      // Http.open("GET",url);
      // Http.send();
