...
function sendData(datavalue) {
  ...
  const url = 'http://biostream-1024.appspot.com/sendps?user=player1&data=0,'+datavalue;
      Http.open("GET",url);
      Http.send();
}
...
if (require.main === module) {
  ...
  client.ready.then(() => client.init()).then(() =>
    numbers(client, avgWindow, averages => {
      ...
      const url = 'http://biostream-1024.appspot.com/sendps?user=player1&data='.concat(allVals);
      Http.open("GET",url);
      Http.send();
